...
import com.hzxckj.common.mybatis.annotation.QueryWhere;
...
public class QuerySupportMethod extends AbstractMethod {
    ...
    protected String[] getSupportMethods(Class<?> mapperClass, Class<?> modelClass, TableInfo tableInfo) {
        QueryWhere support = findAnnotation(mapperClass, QueryWhere.class);
        ...
        if (loadClassMethod) {
            ...
            for (Method method : methods) {
                support = findAnnotation(method, QueryWhere.class);
    ...
    public String getMethodSqlByAnnotation(Class<?> mapperClass, String name) {
        ...
        if (getSqlWithAnnotation) {
            ...
            Select select = findAnnotation(selectMethod, Select.class);
        ...
        for (String method : methodList) {
            ...
            try {
                if (StringUtils.isNotEmpty(beforeSql)) {
                    ...
                    String currentSql = String.format(scriptSql, this.sqlWhereEntityWrapper(true,tableInfo));
                    //移除之前所应用的方法
    ...
    protected MappedStatement addSelectMappedStatement(Class<?> mapperClass, String id, SqlSource sqlSource, Class<?> resultType, TableInfo table) {
        ...
        ResultType resultTypeAnnotation = findAnnotation(method, ResultType.class);
        if (resultTypeAnnotation != null) {
            resultType = resultTypeAnnotation.value();
        } else {

            ResultMap resultMapAnnotation = findAnnotation(method, ResultMap.class);
